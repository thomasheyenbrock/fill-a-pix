<template>
  <div class="block" v-on:click="toggleState" :class="{ filled: block.state === 'filled' }">
    <svg v-if="block.state === 'empty'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" width="32" height="32">
    ...
    <input type="number" min="0" max="9" v-if="gameState === 'setup'" :value="block.number" @input="updateBlockNumber" />
  </div>
</template>
...
export default class Block extends Vue {
  ...
  @Mutation toggleBlockState!: (options: { row: number, column: number }) => void;
